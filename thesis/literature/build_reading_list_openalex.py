...
DEFAULT_QUERIES: list[Query] = [
    Query(
        ...
        "dinosaur body size distribution bimodal skew",
    ),
    Query(
        "dinosaur_body_size_evolution_and_climate",
        "dinosaur body size evolution climate temperature",
    ),
    Query(
        "ecospace_theoretical_and_functional_diversity",
        "theoretical ecospace functional diversity fossil record",
    ),
    Query(
        "functional_convergence_and_homogenization",
        "functional convergence biotic homogenization fossil record",
    ),
    Query(
        "provinciality_fragmentation_and_biogeography",
        "marine provinciality biogeography fragmentation plate tectonics fossil",
    ),
    Query(
        "mass_extinction_functional_structure",
        "mass extinction functional diversity ecospace recovery",
    ),
    Query(
        "climate_volatility_deep_time_biotic_response",
        "climate volatility deep time biotic response Phanerozoic",
    ),
    Query(
        "pbdb_sampling_bias_and_methods",
        "Paleobiology Database sampling bias rock record methods",
    ),
    Query(
        "li_2022_cesm_540myr_dataset",
        "high-resolution climate simulation dataset past 540 million years Li Hu",
    ),
]
...
def _oa_url(work: dict[str, Any]) -> str | None:
    ...
    url = oa.get("oa_url") or oa.get("oa_url") or None
...
def main() -> None:
    ...
    p.add_argument("--max-per-query", type=int, default=300)
    p.add_argument("--max-md-per-query", type=int, default=80)
    args = p.parse_args()
